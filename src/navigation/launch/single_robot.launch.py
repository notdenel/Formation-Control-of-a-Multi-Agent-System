...
Use this on a real Pi alongside the hardware stack.
  ...
  ros2 launch navigation single_robot.launch.py robot_name:=robot1

  # Use a specific map (filename without extension, from config/maps/):
  ros2 launch navigation single_robot.launch.py robot_name:=robot1 map:=map_01
...
def launch_setup(context, *args, **kwargs):
    ...
    if not os.path.isfile(map_yaml):
        raise FileNotFoundError(
            ...
            f"config/maps/{map_name}.yaml inside navigation."
        )
