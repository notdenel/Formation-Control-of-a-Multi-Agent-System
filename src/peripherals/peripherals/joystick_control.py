...
class JoystickController(Node):
    ...
    def axes_callback(self, axes):
        ...
        # For example, use the D-pad to control forward/backward linear velocity(例如，使用十字键来控制前进/后退的线速度)
        if axes['hat_x'] == 1:  # Up(上)
            twist.linear.x = 1.0  # Increase forward speed(增加前进速度)
        elif axes['hat_x'] == -1:  # Down(下)
            twist.linear.x = -1.0  # Decrease forward speed(减少前进速度)

        # Use the D-pad left/right to control robot rotation(使用十字键的左右控制机器人旋转)
        if axes['hat_y'] == 1:  # Right(右)
            twist.linear.y = 1.0  # Increase the rotation speed(增加旋转速度)
        elif axes['hat_y'] == -1:  # Left(左)
            twist.linear.y = -1.0  # Decrease the rotation speed(减少旋转速度)
    ...
    def update_buttons(self):
        try:
            ...
            # Always publish current axes state so the robot keeps moving while
            # a joystick axis is held (no new events = same last_axes values).
