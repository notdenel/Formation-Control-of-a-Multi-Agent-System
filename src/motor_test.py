        ...
        while True:
            if self.enable_recv:
                try:
                    ...
                    print(f"[recv_task] Serial read error: {e}")
                    # Attempt to recover by waiting and reopening
                    self.port.close()
                    time.sleep(1.0)
                    try:
                        self.port.open()
                        print("[recv_task] Port reopened successfully.")
                    except serial.SerialException as reopen_err:
                        print(f"[recv_task] Could not reopen port: {reopen_err}")
                        break  # Give up if we can't reopen
                    continue  # Skip processing, restart loop
...
def move_robot(board, speeds=[[1, 1.0], [2, 1.0], [3, 1.0], [4, 1.0]], duration=2):
    try:
...
def teleop_turn(board):
    print("Teleop mode active. Use Left Joystick (Left/Right) to rotate.")
    ...
    while True:
        ...
        if data is not None:
            axes, buttons = data
            # axes[0] is typically the Left Stick X-axis (range -1.0 to 1.0)
            turn_power = axes[0] 
            
            # Deadzone check (prevents drifting if the stick is slightly off-center)
            if abs(turn_power) < 0.1:
                turn_power = 0
            
            # 2. Map turn_power to motor speeds
            # Motor IDs: 1 & 2 (Left), 3 & 4 (Right) 
            # Note: Depending on your wiring, you might need to swap the '-' sign
            left_speed = turn_power
            right_speed = -turn_power
...
#     ###MAX SPEED IS 5.0, IDIOT!!!!
    
#     #conversion: 1.0 -> 20cm/s
...
#     # move_robot(board, [[1, -1.0], [2, -1.0], [3, 1.0], [4, 1.0]], duration=3)    
#     # move_robot(board, [[1, -2.0], [2, -2.0], [3, 2.0], [4, 2.0]], duration=3)
#     # move_robot(board, [[1, -3.0], [2, -3.0], [3, 3.0], [4, 3.0]], duration=3)
#     # move_robot(board, [[1, -4.0], [2, -4.0], [3, 4.0], [4, 4.0]], duration=3)
